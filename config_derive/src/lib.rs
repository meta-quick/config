//! A derive macro that implements `MetaConfig` trait.
//! For complete documentation please see [config](https://docs.rs/MetaConfig).
...
fn impl_envconfig(input: &DeriveInput) -> proc_macro2::TokenStream {
    ...
    let inner_impl = match input.data {
        Struct(ref ds) => match ds.fields {
            ...
            _ => panic!("config supports only named fields"),
        },
        _ => panic!("config only supports non-tuple structs"),
    };
...
fn impl_envconfig_for_struct(
    ...
    quote! {
        impl MetaConfig for #struct_name {
            ...
            fn init_from_hashmap(hashmap: &std::collections::HashMap<String, String>) -> ::std::result::Result<Self, ::config::Error> {
                let config = Self {
...
fn gen_field_assign_for_optional_type(
    ...
    match source {
        Source::Environment => quote! {
                #ident: ::config::load_optional_var(#from, None)?
        },
        Source::HashMap => quote! {
            #ident: ::config::load_optional_var(#from, Some(hashmap))?
        },
    }
}
...
fn gen_field_assign_for_non_optional_type(
    ...
    if let Some(default) = opt_default {
        match source {
            Source::Environment => quote! {
                #ident: ::config::load_var_with_default(#from, None, #default)?
            },
            Source::HashMap => quote! {
                #ident: ::config::load_var_with_default(#from, Some(hashmap), #default)?
            },
        }
    } else {
        match source {
            Source::Environment => quote! {
                #ident: ::config::load_var(#from, None)?
            },
            Source::HashMap => quote! {
                #ident: ::config::load_var(#from, Some(hashmap))?
            },
        }
    }
}
...
fn fetch_list_from_attr(field: &Field, attr: &Attribute) -> Punctuated<NestedMeta, Comma> {
    let opt_meta = attr.parse_meta().unwrap_or_else(|err| {
        panic!(
            "Can not interpret meta of `config` attribute on field `{}`: {}",
            field_name(field),
            err
        )
    });
    ...
    match opt_meta {
        ...
        _ => panic!(
            "`config` attribute on field `{}` must contain a list",
            field_name(field)
        ),
    }
}
    ...
    list.iter()
        .map(|item| match item {
            NestedMeta::Meta(meta) => match meta {
                ...
                _ => panic!(
                    "`config` attribute on field `{}` must contain name/value item",
                    field_name(field)
                ),
            },
            _ => panic!(
                "Failed to process `config` attribute on field `{}`",
                field_name(field)
            ),
        })
